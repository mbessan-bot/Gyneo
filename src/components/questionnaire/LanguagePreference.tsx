import { Label } from "@/components/ui/label";
import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group";

interface LanguagePreferenceProps {
  value: string;
  onChange: (value: string) => void;
}

export const LanguagePreference = ({ value, onChange }: LanguagePreferenceProps) => {
  const languages = [
    { value: "english", label: "English" },
    { value: "spanish", label: "Spanish" },
    { value: "french", label: "French" },
    { value: "mandarin", label: "Mandarin" },
    { value: "arabic", label: "Arabic" },
    { value: "other", label: "Other" },
    { value: "no-preference", label: "No preference" },
  ];

  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold mb-3 bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">
          Language Preference
        </h2>
        <p className="text-muted-foreground text-lg">
          In which language would you prefer to communicate?
        </p>
      </div>

      <RadioGroup value={value} onValueChange={onChange} className="space-y-4">
        {languages.map((language) => (
          <div
            key={language.value}
            className="flex items-center space-x-3 p-4 rounded-lg border-2 border-border hover:border-primary transition-all cursor-pointer has-[:checked]:border-primary has-[:checked]:bg-primary/5"
          >
            <RadioGroupItem value={language.value} id={language.value} />
            <Label htmlFor={language.value} className="flex-1 cursor-pointer text-base font-medium">
              {language.label}
            </Label>
          </div>
        ))}
      </RadioGroup>
    </div>
  );
};
